...
const [showItemForm, setShowItemForm] = useState(null);
    ...
    const list = (dashboard?.portfolios || []).map((p) => {
      ...
      const totalCost = items.reduce((s, i) => s + SafeNumber(i.buy_price) * SafeNumber(i.quantity), 0);
       const totalPL = items.reduce((s, i) => {
         ...
         const sellTotal = i.sell_price ? SafeNumber(i.sell_price) * qty : 0;
         const sellComm = commissionEnabled && i.sell_price ? sellTotal * sellCommissionRate : 0;
  ...
  }, [dashboard, portfolioSort, commissionEnabled, buyCommissionRate, sellCommissionRate]);
  const allItems = useMemo(() => {
    const activePortfolios = showInactivePortfolios ? portfolios : portfolios.filter((p) => p.active !== false && p.active !== 0);
    return activePortfolios.flatMap((p) => p.items || []);
  }, [portfolios, showInactivePortfolios]);
   ...
   const totals = useMemo(() => {
    ...
    const totalPL = portfolios.reduce((s, p) => s + SafeNumber(p.total_profit_loss), 0);
    const soldCost = portfolios.reduce((s, p) => {
      const cost = (p.items || []).reduce((sum, item) => {
        if (item.sell_price && item.sell_price > 0) {
          return sum + SafeNumber(item.buy_price) * SafeNumber(item.quantity);
        }
        return sum;
      }, 0);
      return s + cost;
  ...
  }, [portfolios, allItems, commissionEnabled, buyCommissionRate, sellCommissionRate]);
  ...
  const handleCopy = (e, value) => {
    ...
    const raw = isNaN(num) ? String(value) : String(Math.round(num));
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 flex-wrap">
 {[
            { label: 'پرتفوها', value: totals.count, format: 'num', icon: FolderOpen },
            { label: 'سهم‌ها', value: totals.itemCount, format: 'num', icon: Package },
             { label: 'ارزش کل خرید', value: totals.totalCost, format: 'price', icon: Wallet },
             { label: 'ارزش کل پرتو', value: totals.totalValue, format: 'price', icon: Wallet },
             { label: 'سود کل', value: totals.totalPL, format: 'pl', positive: totals.totalPL >= 0, icon: totals.totalPL >= 0 ? TrendingUp : TrendingDown },
             { label: 'سود/ضرر', value: totals.totalPLPct, format: 'pct', positive: totals.totalPL >= 0, icon: totals.totalPL >= 0 ? TrendingUp : TrendingDown },
          ].map((s) => {
           const Icon = s.icon;
           return (
           <div key={s.label} className="flex-1 min-w-[100px] bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-lg px-3 py-2">
             ...
             <p className={`text-sm font-bold mt-0.5 ${s.format === 'pl' && typeof s.value === 'number' && !isNaN(s.value) ? (s.value >= 0 ? 'text-success' : 'text-danger') : s.format === 'pct' && typeof s.value === 'number' && !isNaN(s.value) ? (s.value >= 0 ? 'text-success' : 'text-danger') : 'text-slate-800 dark:text-slate-200'}`}>
               {s.format === 'num' ? s.value.toLocaleString('fa-IR') : s.format === 'price' && typeof s.value === 'number' && !isNaN(s.value)
                  ? (unit === 'toman' ? s.value / 10 : s.value).toLocaleString('fa-IR', { minimumFractionDigits: 0, maximumFractionDigits: 0 }) + ' ' + (unit === 'toman' ? 'تومان' : 'ریال')
                  : s.format === 'pl' && typeof s.value === 'number' && !isNaN(s.value)
                  ? (unit === 'toman' ? s.value / 10 : s.value).toLocaleString('fa-IR', { minimumFractionDigits: 0, maximumFractionDigits: 0 }) + ' ' + (unit === 'toman' ? 'تومان' : 'ریال')
                 : s.format === 'pct' && typeof s.value === 'number' && !isNaN(s.value)
                 ? s.value.toLocaleString('fa-IR', { minimumFractionDigits: 0, maximumFractionDigits: 2 }) + ' درصد'
                 : '—'}
             </p>
           </div>
           );
       ...
       <div className="relative bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-sm">
         <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
           <div>
             <h3 className="pr-8 text-sm font-semibold text-slate-700 dark:text-slate-200 rtl-text flex items-center gap-1.5">
               ...
               {activeChart === 'allocation' ? 'ترکیب ارزش دارایی‌ها' : activeChart === 'price' ? 'نمودار قیمت‌ها' : 'سود و ضرر'}
             </h3>
         ...
         {chartsExpanded && (
            <div className="flex items-center gap-4 mb-2 flex-row-reverse">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    setShowInactivePortfolios(next);
                    if (next) {
              ...
              {(activeChart === 'price' || activeChart === 'profitLoss') && (
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    disabled={showInactivePortfolios}
                    onClick={() => {
                    ...
                    className={`relative w-9 h-5 rounded-full transition-colors ${showInactiveChartItems ? 'bg-brand-500' : 'bg-slate-300 dark:bg-slate-700'} ${showInactivePortfolios ? 'opacity-60 cursor-not-allowed' : ''}`}
                  >
      ...
      <div className="flex items-center justify-between">
         <h2 className="text-sm font-bold text-slate-800 dark:text-slate-200 rtl-text flex items-center gap-2">
           ...
           پرتفوها
             <span className="text-[10px] text-slate-400 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded-full">{portfolios.length.toLocaleString('fa-IR')}</span>
         </h2>
        ...
        {portfolios.map((portfolio) => {
            ...
            const pi = [...(portfolio.items || [])].sort((a, b) => {
              if (sortConfig.key) {
                ...
                switch (sortConfig.key) {
                  ...
                  case 'pl': aVal = SafeNumber(a.sell_price) > 0 ? (SafeNumber(a.sell_price) - SafeNumber(a.buy_price)) * SafeNumber(a.quantity) : 0; bVal = SafeNumber(b.sell_price) > 0 ? (SafeNumber(b.sell_price) - SafeNumber(b.buy_price)) * SafeNumber(b.quantity) : 0; break;
                  case 'pct': aVal = SafeNumber(a.sell_price) > 0 ? ((SafeNumber(a.sell_price) - SafeNumber(a.buy_price)) / SafeNumber(a.buy_price)) * 100 : 0; bVal = SafeNumber(b.sell_price) > 0 ? ((SafeNumber(b.sell_price) - SafeNumber(b.buy_price)) / SafeNumber(b.buy_price)) * 100 : 0; break;
          ...
          return (
            <div key={portfolio.id} className={`bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-lg overflow-hidden transition-shadow hover:shadow-sm ${!portfolio.active && showItemForm !== portfolio.id ? 'opacity-50' : ''}`}>
              {/* Portfolio Header */}
              <div className="flex items-center justify-between px-3 py-2.5 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors" onClick={() => togglePortfolio(portfolio.id)}>
                <div className="flex items-center gap-2 min-w-0 flex-1">
                  ...
                  <span className="text-sm font-medium text-slate-800 dark:text-slate-200 rtl-text truncate">{portfolio.name}</span>
                   <span className="text-[10px] text-slate-400 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded-full shrink-0">{pi.length.toLocaleString('fa-IR')} سهم</span>
                </div>
                  <div className="flex items-center gap-3 shrink-0 mr-2">
                    <div className="flex flex-col items-center gap-0.5">
                      <span className="text-[10px] text-slate-400 dark:text-slate-500">ارزش خرید</span>
                      <span className="text-xs font-medium text-slate-600 dark:text-slate-300">{(unit === 'toman' ? pc / 10 : pc).toLocaleString('fa-IR', { minimumFractionDigits: 0, maximumFractionDigits: 0 })} {unit === 'toman' ? 'تومان' : 'ریال'}</span>
                    </div>
                    ...
                    <div className="flex flex-col items-center gap-0.5">
                      ...
                      <span className="text-xs font-medium text-slate-700 dark:text-slate-300">{(unit === 'toman' ? pv / 10 : pv).toLocaleString('fa-IR', { minimumFractionDigits: 0, maximumFractionDigits: 0 })} {unit === 'toman' ? 'تومان' : 'ریال'}</span>
                    </div>
                    <div className="w-px h-4 bg-slate-200 dark:bg-slate-700 shrink-0" />
                    <div className="flex flex-col items-center gap-0.5">
                      <span className="text-[10px] text-slate-400 dark:text-slate-500">مبلغ سود و زیان (درصد)</span>
                      <span className={`text-xs font-bold ${pp >= 0 ? 'text-success' : 'text-danger'}`}>{(unit === 'toman' ? SafeNumber(portfolio._profitLoss) / 10 : SafeNumber(portfolio._profitLoss)).toLocaleString('fa-IR', { minimumFractionDigits: 0, maximumFractionDigits: 0 })} {unit === 'toman' ? 'تومان' : 'ریال'} {pp !== 0 && `(${pp.toLocaleString('fa-IR', { minimumFractionDigits: 0, maximumFractionDigits: 1 })} درصد)`}</span>
                    </div>
                    <div className="w-px h-4 bg-slate-200 dark:bg-slate-700 shrink-0" />
                ...
                <div className="border-t border-slate-100 dark:border-slate-800 animate-fade-in">
                  ...
                  {pi.length > 0 && (
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-[11px]">
                         <thead>
                            <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/30">
                              ...
                              <th onClick={() => toggleSort('pl')} className="px-2 py-1.5 text-right font-medium text-slate-400 rtl-text cursor-pointer hover:text-slate-600 dark:hover:text-slate-300 select-none">{sortLabel('pl', 'سود/ضرر')}</th>
                         ...
                         <tbody>
                            {pi.map((item) => {
                              ...
                              const displayPrice = isRealized ? sellN : lastN;
                              ...
                              const pl = plAmount !== null && purchaseValue > 0 ? (plAmount / purchaseValue) * 100 : null;
                                return (
                                   <tr key={item.id} className={`border-b border-slate-50/50 dark:border-slate-800/50 hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors ${!item.active && portfolio.active !== false && portfolio.active !== 0 ? 'opacity-40' : ''}`}>
                                   <td className="px-2 py-1.5 font-medium text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
                                    ...
                                    <td className="px-2 py-1.5 font-medium text-slate-800 dark:text-slate-200 cursor-pointer" onDoubleClick={(e) => handleCopy(e, item.last_price)}>{item.last_price ? formatPrice(item.last_price, unit, 2) : '—'}</td>
                                     <td className="px-2 py-1.5 font-medium text-slate-800 dark:text-slate-200 cursor-pointer" onDoubleClick={(e) => handleCopy(e, item.pe)}>{item.pe ? formatPE(item.pe) : '—'}</td>
                                    ...
                                    <td className="px-2 py-1.5 text-slate-500 cursor-pointer" onDoubleClick={(e) => handleCopy(e, item.quantity)}>{formatNumber(item.quantity)}</td>
                                    <td className="px-2 py-1.5 text-slate-500 cursor-pointer" onDoubleClick={(e) => handleCopy(e, item.sell_price)}>{item.sell_price ? formatPrice(item.sell_price, unit, 2) : '—'}</td>
                                     ...
                                     <td className={`px-2 py-1.5 font-medium cursor-pointer ${plAmount !== null ? (plAmount >= 0 ? 'text-success' : 'text-danger') : 'text-slate-300'}`} onDoubleClick={(e) => handleCopy(e, plAmount)}>
                                       {plAmount !== null ? formatPrice(plAmount, unit) : '—'}
                                   </td>
                                    <td className="px-2 py-1.5">
                                      {pl != null ? (
                                        <span className={`flex items-center gap-0.5 font-medium ${pl >= 0 ? 'text-success' : 'text-danger'}`}>
                                           {formatPercent(pl)}
                                        </span>
                      ...
                      {pi.some((item) => !item.sell_price && item.last_price) && (
                        <div className="px-3 py-1 text-[9px] text-muted-foreground border-t border-slate-100 dark:border-slate-800">
                          <span className="text-slate-400 font-bold">●</span> سود/ضرر و درصد بر اساس آخرین قیمت (محقق نشده) — در محاسبه کل لحاظ نمی‌شود
                        </div>
                      )}
                    </>
                  )}
...
function PriceChart({ items, unit }) {
  const chartData = useMemo(() => {
    ...
    const validItems = items.filter((i) => i.sell_price && i.sell_price > 0 && i.buy_price != null && i.symbol);
    if (validItems.length === 0) return null;
    const labels = validItems.map((i) => i.symbol);
    const data = validItems.map((i) => SafeNumber(i.buy_price));
    return {
      labels,
      datasets: [
        { label: 'خرید', data, borderColor: '#EF4444', backgroundColor: 'rgba(239,68,68,0.10)', fill: true, tension: 0.4, pointRadius: 2, pointHoverRadius: 4, borderWidth: 2 },
        { label: 'فروش', data: validItems.map((i) => SafeNumber(i.sell_price)), borderColor: '#10B981', backgroundColor: 'transparent', borderDash: [4, 4], tension: 0.4, pointRadius: 2, borderWidth: 1.5 },
      ],
    };
  }, [items]);
  const options = useMemo(() => ({
    ...
    plugins: {
      ...
      tooltip: { direction: 'rtl', displayColors: true, padding: 10, cornerRadius: 8, bodyFont: { size: 11, family: "'Vazirmatn', system-ui, sans-serif" }, backgroundColor: '#0F172A', borderColor: 'transparent', borderWidth: 0, callbacks: { label: (ctx) => { const value = unit === 'toman' ? (ctx.parsed?.y ?? 0) / 10 : (ctx.parsed?.y ?? 0); return `${ctx.dataset.label}: ${value.toLocaleString('fa-IR', { maximumFractionDigits: 0 })} ${unit === 'toman' ? 'تومان' : 'ریال'}`; } } },
    },
    scales: {
...
function PLChart({ items, showAmount, unit }) {
  const chartData = useMemo(() => {
    const valid = items.filter((i) => i.sell_price && i.sell_price > 0 && i.buy_price && i.buy_price > 0);
    if (valid.length === 0) return null;
    const percentages = valid.map((i) => SafeNumber(((i.sell_price - i.buy_price) / i.buy_price * 100).toFixed(1)));
    const amounts = valid.map((i) => {
      const amount = (SafeNumber(i.sell_price) - SafeNumber(i.buy_price)) * SafeNumber(i.quantity);
